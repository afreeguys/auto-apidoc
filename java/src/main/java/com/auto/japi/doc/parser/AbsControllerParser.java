...
import com.auto.japi.doc.Ignore;
import com.auto.japi.doc.ParseUtils;
import com.auto.japi.doc.Utils;
import com.auto.japi.doc.consts.ChangeFlag;
...
public abstract class AbsControllerParser {
     ...
     * 获取jsonObject 属性
     */
    void parseClassNodeByMethodBodyForJSONObject(MethodDeclaration methodDeclaration, ParamNode paramNode, ClassNode classNode) {
        ...
        statements.forEach(statement -> {
            ...
            String indentifyStr = paramNode.getName() + ".get";
            int index = s.indexOf(indentifyStr);
            if (index < 0) {
                return;
            }
            index += indentifyStr.length();
            String type = s.substring(index, index + 3);
            if ("Int".equals(type)) {
                index += 5;
                String fieldName = s.substring(index, s.indexOf("\"", ++index));
                FieldNode fieldNode = new FieldNode();
                fieldNode.setType("int");
                fieldNode.setName(fieldName);
                fieldNode.setDescription(fieldName);
